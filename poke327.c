...
int movement_costs[NUM_TRAINER_TYPES][NUM_TERRAIN_TYPES] = {
    // Bldr, Tree, Path, PMart, PCntr, TGras, SGras, Mtn, Forest, Water, Gate
    ...
    {INT_MAX, INT_MAX, INT_MAX, INT_MAX, INT_MAX, INT_MAX, INT_MAX, INT_MAX, INT_MAX, 7, INT_MAX}, // Swimmer
    {INT_MAX, INT_MAX, 10, 50, 50, 20, 10, INT_MAX, INT_MAX, INT_MAX, INT_MAX}  // Other
};
...
Node* createNode(int row, int col, int distance) {
...
static void dijkstra(map_t *m) {
    // Initialize the heap
    heap_t heap;
    heap_init(&heap, compareNodes, NULL);

    // Randomly place the PC on a road
    int pcRow, pcCol;
    while (1) {
        pcRow = rand() % ROWS;
        pcCol = rand() % COLS;
        if (m->map[pcRow][pcCol] == '#') {
            break;
        }
    }

    // Initialize distances array and set all distances to INT_MAX
    ...
    // Set the PC's initial position and distance
    distances[pcRow][pcCol] = 0;

    // Insert the PC's position into the heap
    Node *startNode = (Node *)malloc(sizeof(Node));
    startNode->row = pcRow;
    startNode->col = pcCol;
    startNode->distance = 0;
    heap_insert(&heap, startNode);

    // Dijkstra's Algorithm
    while (heap.size > 0) {
        // Extract the node with the minimum distance
        Node *currentNode = (Node *)heap_remove_min(&heap);
        ...
        // Update the distances of neighboring cells
        for (int i = 0; i < 4; i++) {
            ...
            if (newRow >= 0 && newRow < ROWS && newCol >= 0 && newCol < COLS) {
                int newTerrain = m->map[newRow][newCol];
                int newCost = movement_costs[PC][newTerrain]; // Assuming PC is the moving entity

                // Check if a shorter path to the new cell is found
                if (distances[currentRow][currentCol] + newCost < distances[newRow][newCol]) {
                    distances[newRow][newCol] = distances[currentRow][currentCol] + newCost;

                    // Insert the new position into the heap
                    Node *newNode = (Node *)malloc(sizeof(Node));
                    newNode->row = newRow;
                    newNode->col = newCol;
                    newNode->distance = distances[newRow][newCol];
    ...
    // No return statement needed as the function is void
}

void displayDistanceMap(int distances[ROWS][COLS]) {
    for (int i = 0; i < ROWS; ++i) {
        for (int j = 0; j < COLS; ++j) {
            printf("%2d ", distances[i][j] % 100);
        }
        printf("\n");
    }
}
...
int main(int argc, char *argv[])
{
  ...
  int hikerDistances[ROWS][COLS];
  int rivalDistances[ROWS][COLS];
